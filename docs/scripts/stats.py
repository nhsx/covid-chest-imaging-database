...
if WAREHOUSE_BUCKET is None:
    sys.exit("No bucket info is provided (via the AWS_WAREHOUSE_BUCKET env var)")
...
with open("../public/data/stats_patients.csv", "w") as csvfile:
...
with open("../public/data/stats_images.csv", "w") as csvfile:
    statswriter = csv.writer(
        csvfile, delimiter=",", quotechar='"', quoting=csv.QUOTE_MINIMAL
    )
    statswriter.writerows(output)
...
output += [["Clinical data and image metadata export", f"{remainder_storage:,.0f}GB"]]
with open("../public/data/stats_storage.csv", "w") as csvfile:
    statswriter = csv.writer(
        csvfile, delimiter=",", quotechar='"', quoting=csv.QUOTE_MINIMAL
    )
    statswriter.writerows(output)
...
output = [
   [
      "Submitting centres (e.g. NHS trusts)", 
      f"{nhs_trusts:,.0f}"
   ]
]
with open("../public/data/stats_submittingcentres.csv", "w") as csvfile:
    statswriter = csv.writer(
        csvfile, delimiter=",", quotechar='"', quoting=csv.QUOTE_MINIMAL
    )
    statswriter.writerows(output)

with open("../public/data/stats_date.txt", "w") as fragment:
